...
def manual_strategy_screen(stdscr):
    curses.curs_set(0)
    stdscr.timeout(-1)
    ...
    def refresh():
        ...
        for screen_row in range(max_visible):
            ...
            if real_idx >= len(configs):
                break

            y = 5 + screen_row
            num = real_idx + 1
            text = f"{num:3d}. {configs[real_idx]}"
    ...
    while True:
        ...
        if key in (curses.KEY_UP, ord('k')):
            ...
            selected_strategy = configs[current_idx]
            ...
            try:
                ...
                msg = f"Применена стратегия: {selected_strategy}"
